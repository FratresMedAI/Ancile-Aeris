...
class DarkSpaceAdapterNode(Node):
    def __init__(self) -> None:
        super().__init__("darkspace_adapter_node")
        ...
        self.declare_parameter("health_topic", "/darkspace/health")
        self.declare_parameter("db_path", "/tmp/darkspace_adapter.db")
        self.declare_parameter("hmac_secret_env", "DARKSPACE_HMAC_SECRET")
    ...
    def _init_db(self) -> None:
        ...
        conn.execute(
            """
            CREATE TABLE IF NOT EXISTS darkspace_bridge_log (
    ...
    def _store(self, topic: str, payload: str) -> None:
        ...
        conn = self._conn()
        conn.execute(
            "INSERT INTO darkspace_bridge_log (ts, source_topic, payload_json, payload_hash, hmac_sig) VALUES (?, ?, ?, ?, ?)",
    ...
    def _publish_health(self) -> None:
        conn = self._conn()
        row = conn.execute("SELECT COUNT(*) FROM darkspace_bridge_log").fetchone()
        conn.close()
