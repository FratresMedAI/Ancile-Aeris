...
def _launch_fob_swarm(context, *args, **kwargs):
    share_dir = get_package_share_directory("clearsky_os_scout_mothership")
    ...
    for i in range(count):
        ...
        nodes.append(
            Node(
                package="clearsky_os_scout_mothership",
                executable="clearsky_os_scout_mothership_node",
                name=f"clearsky_os_scout_mothership_{i+1:03d}",
                output="screen",
                prefix="python3",
                parameters=[
                    config_yaml,
