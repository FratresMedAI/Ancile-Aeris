...
from operator_copilot_node.srv import QueryCopilot
...
class OperatorCopilotNode(Node):
    def __init__(self) -> None:
        super().__init__("operator_copilot_node")
        self.declare_parameter("backend", "template")
        self.declare_parameter("dashboard_state_file", "/tmp/counterdrone_dashboard_state.json")
        self.guard = _load_guard()
        self.service = self.create_service(QueryCopilot, "operator_copilot/query", self._on_query)
        self.get_logger().info("operator_copilot_node initialized")
