#!/usr/bin/env python3
"""Ancile Aeris high-altitude ISR scout mothership with mesh coordination (C-UAS defensive-only)."""
...
def _arson_carrier_counterterror_signal(sim_phase: int) -> Dict[str, Any]:
    """Simulation-only counter-terror arson / incendiary precursor pattern (defensive classification)."""
    hot = (sim_phase % 7) == 0
    return {
...
class ScoutMothershipNode(Node):
    def __init__(self) -> None:
        ...
        self.get_logger().info("scout_mothership_node initialized (mesh coordination enabled if configured)")
    ...
    def _prune_peers(self) -> None:
        now = time.monotonic()
        stale = [k for k, t in self._peer_seen.items() if now - t > 15.0]
        for k in stale:
            del self._peer_seen[k]

    def _publish_mesh_swarm_status(self, coverage_cell: str) -> None:
        if not bool(self.get_parameter("enable_mesh_publish").value):
            return
        self._prune_peers()
        ...
        mesh_payload = {
            ...
            "coordinated_coverage": {
                ...
                "redundancy_factor": max(1, int(self.get_parameter("mesh_neighbor_count_hint").value)),
            },
            "members": members,
    ...
    def _publish_handoff(self, track_id: str) -> None:
        ...
        self.audit_bridge.emit(
            "scout_handoff",
            handoff,
            xai_text="Mesh-capable mothership ISR handoff simulation; kinetic release remains strictly human gated.",
        )
