...
from clearsky_os_operator_copilot.srv import QueryCopilot
...
class OperatorCopilotNode(Node):
    def __init__(self) -> None:
        super().__init__("clearsky_os_operator_copilot_node")
        self.declare_parameter("backend", "template")
        self.declare_parameter("dashboard_state_file", "/tmp/clearsky_os_dashboard_state.json")
        self.guard = _load_guard()
        self.service = self.create_service(QueryCopilot, "clearsky_os_operator_copilot/query", self._on_query)
        self.query_copilot_alias = self.create_service(QueryCopilot, "QueryCopilot", self._on_query)
        self.get_logger().info("clearsky_os_operator_copilot_node initialized")

    def _on_query(self, request: QueryCopilot.Request, response: QueryCopilot.Response) -> QueryCopilot.Response:
        backend = str(os.getenv("CLEARSKY_COPILOT_BACKEND", self.get_parameter("backend").value))
        ...
        if self.guard is not None:
            ...
            if verdict.label == "block":
                ...
                response.reason = "clearsky_rule_guard_block"
                response.answer = "Query blocked by ClearSky OS safety rule guard."
                return response
        ...
        response.reason = "ok"
        if backend == "ollama":
            response.answer = "Ollama backend placeholder: falling back to template summary. " + _template_answer(query, state)
        else:
