...
| **Interoperability** | JSON contracts map to Lattice-style mesh overlays and PEO UAS/C-UAS dashboards. |
...
- **Directed-energy analogs** cited in documentation only; software stack never arms HPM autonomously—human-on-loop maintained.

## Proof Points (Baseline Demo)

```text
ros2 topic list | grep -E 'fused_tracks|audit/events|mesh|interceptor_status'
ros2 service list | grep QueryCopilot
```
