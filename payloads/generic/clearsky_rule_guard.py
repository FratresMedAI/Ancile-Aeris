"""
Stateless safeguard scoring: ClearSky OS safeguard rebuff + enforcer patterns without SQLite.
...
_INJECTION_PATTERNS: list[tuple[str, float]] = [
    ...
    (r"(?is)update_ticket[\s\S]{0,260}(sensitive\s+data\s+redacted|status['\"]?\s*,?\s*new_value['\"]?\s*[:=]\s*['\"]?closed|field_name['\"]?\s*[:=]\s*['\"]?status)", 8.9),
    ...
    (r"(?i)goodwill\s+compensation\s+guideline", 8.8),
    (r"(?i)fraud\s+flag\s+criteria", 8.8),
    ...
    (r"(?is)t[o0]{2}l.{0,120}(?:i|1|l)n(?:t|7|T)e(?:r|R){1,2}", 8.7),
]
